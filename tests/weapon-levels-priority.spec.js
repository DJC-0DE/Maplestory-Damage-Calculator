...
test.describe("Weapon Levels - Upgrade Priority Algorithm", () => {
  ...
  test.describe("Priority Chain Generation", () => {
    test("empty weapons state shows no weapons available message", async ({
      page,
    }) => {
      // Arrange
      ...
      await page
        .locator("#weapon-levels-subtab-button")
        .filter({ hasText: "Upgrade Priority" })
        .click();
      await page.waitForTimeout(200);
      ...
      await expect(chainElement).toContainText(
        "No weapons available to upgrade",
      );
    ...
    test("priority chain generates with configured weapons", async ({
      page,
    }) => {
      // Arrange
    ...
    test("priority chain updates when weapon levels change", async ({
      page,
    }) => {
      // Arrange
      await applyWeaponLevelsFixture(page, WEAPON_LEVELS_EARLY);
    ...
    test("priority chain prioritizes highest efficiency weapon", async ({
      page,
    }) => {
      // Arrange
      await page.goto(`${BASE_URL}/#/setup/weapon-levels`);
    ...
    test("priority chain shows dividers between weapon types", async ({
      page,
    }) => {
      // Arrange
      await applyWeaponLevelsFixture(page, WEAPON_LEVELS_MID);
  ...
  test.describe("Currency Calculator", () => {
    ...
    test("currency calculator shows upgrade path with counts", async ({
      page,
    }) => {
      // Arrange
      await applyWeaponLevelsFixture(page, WEAPON_LEVELS_EARLY);
    ...
    test("upgrade path shows final weapon levels in parentheses", async ({
      page,
    }) => {
      // Arrange
      await applyWeaponLevelsFixture(page, WEAPON_LEVELS_EARLY);
    ...
    test("apply upgrades button is hidden when no upgrades possible", async ({
      page,
    }) => {
      // Arrange
      await page.goto(`${BASE_URL}/#/setup/weapon-levels`);
    ...
    test("results hide when currency set to 0", async ({ page }) => {
      ...
      await page.locator("#upgrade-currency-input").fill("5000");
      await page.waitForTimeout(300);
      await expect(page.locator("#currency-upgrade-results")).toHaveClass(
        /visible/,
      );
    ...
    test("upgrade path separates weapon types with commas", async ({
      page,
    }) => {
      // Arrange
      await applyWeaponLevelsFixture(page, WEAPON_LEVELS_MID);
  ...
  test.describe("Mathematical Accuracy", () => {
    test("priority algorithm efficiency matches weapon display efficiency", async ({
      page,
    }) => {
      // Arrange
      await page.goto(`${BASE_URL}/#/setup/weapon-levels`);
      ...
      const normalUpgradeGain = await page
        .locator("#upgrade-gain-normal-t4")
        .textContent();
      const rareUpgradeGain = await page
        .locator("#upgrade-gain-rare-t4")
        .textContent();
    ...
    test("currency calculator DPS gain matches actual damage calculation", async ({
      page,
    }) => {
      ...
      const dpsGainText = await page
        .locator("#currency-dps-gain")
        .textContent();
    ...
    test("efficiency calculation accounts for equipped weapon bonus", async ({
      page,
    }) => {
      // Arrange
      await page.goto(`${BASE_URL}/#/setup/weapon-levels`);
      ...
      const upgradeGainText = await page
        .locator("#upgrade-gain-normal-t4")
        .textContent();
    ...
    test("upgrade chain uses same efficiency calculation as display", async ({
      page,
    }) => {
      ...
      const normalGain = await page
        .locator("#upgrade-gain-normal-t4")
        .textContent();
      const rareGain = await page
        .locator("#upgrade-gain-rare-t3")
        .textContent();
  ...
  test.describe("Priority Sub-Tab Interactions", () => {
    test("switching to weapons grid and back maintains calculator state", async ({
      page,
    }) => {
      // Arrange
      await applyWeaponLevelsFixture(page, WEAPON_LEVELS_MID);
      ...
      await expect(page.locator("#upgrade-currency-input")).toHaveValue(
        "10000",
      );
      await expect(page.locator("#currency-upgrade-results")).toHaveClass(
        /visible/,
