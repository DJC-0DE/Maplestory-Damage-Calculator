...
test.describe("Stat Equivalency - Input Field Presence and Validation", () => {
  ...
  test("input fields accept numeric values", async ({ page }) => {
    ...
    // Assert - Value accepted
    await expect(page.locator(EQUIVALENCY_INPUTS.skillCoeff)).toHaveValue(
      "1.5",
    );
  });
  ...
  test("input fields handle decimal values correctly", async ({ page }) => {
    ...
    await expect(page.locator(EQUIVALENCY_INPUTS.damageAmp)).toHaveValue(
      "1.35",
    );
    ...
    // Assert
    await expect(page.locator(EQUIVALENCY_INPUTS.skillMastery)).toHaveValue(
      "0.75",
    );
  });
...
test.describe("Stat Equivalency - Real-Time Calculation", () => {
  ...
  test("entering attack value calculates equivalent gains for other stats", async ({
    page,
  }) => {
  ...
  test("entering main stat value calculates equivalent attack", async ({
    page,
  }) => {
  ...
  test("entering boss damage calculates equivalents for other stats", async ({
    page,
  }) => {
...
test.describe("Stat Equivalency - Cross-Stat Conversion", () => {
  ...
  test("boss damage to normal damage conversion reflects target content", async ({
    page,
  }) => {
  ...
  test("crit rate to crit damage conversion shows diminishing returns", async ({
    page,
  }) => {
    ...
    const lowCritRateToAttack = await page.inputValue(
      EQUIVALENCY_INPUTS.attack,
    );
    ...
    const highCritRateToAttack = await page.inputValue(
      EQUIVALENCY_INPUTS.attack,
    );
  ...
  test("attack speed conversion shows appropriate scaling", async ({
    page,
  }) => {
    ...
    const higherAttackEquivalent = await page.inputValue(
      EQUIVALENCY_INPUTS.attack,
    );
  ...
  test("final damage conversion is calculated correctly", async ({ page }) => {
    ...
    // Assert - Equivalents calculated
    const attackEquivalent = await page.inputValue(EQUIVALENCY_INPUTS.attack);
    const mainStatEquivalent = await page.inputValue(
      EQUIVALENCY_INPUTS.mainStat,
    );
  ...
  test("defense penetration conversion reflects target defense", async ({
    page,
  }) => {
...
test.describe("Stat Equivalency - Input Validation and Edge Cases", () => {
  ...
  test("rapid input changes maintain calculation integrity", async ({
    page,
  }) => {
...
test.describe("Stat Equivalency - Integration with Base Stats", () => {
  ...
  test("changing base stats updates equivalency calculations", async ({
    page,
  }) => {
  ...
  test("switching classes updates equivalency calculations", async ({
    page,
  }) => {
    ...
    await page.fill(EQUIVALENCY_INPUTS.attack, "100");
    await page.waitForTimeout(200);
    const nightLordMainStat = await page.inputValue(
      EQUIVALENCY_INPUTS.mainStat,
    );
  ...
  test("equivalency tab persists state when switching to stat tables", async ({
    page,
  }) => {
    // Arrange
    await applyBaseStatsFixture(page, HERO_LEVEL_100);
  ...
  test("full workflow: configure → navigate to equivalency", async ({
    page,
  }) => {
    // Arrange - Configure character
