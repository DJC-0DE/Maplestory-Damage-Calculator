...
test.describe("Weapon Levels - Main User Workflows", () => {
  ...
  test.describe("First-Time Setup Workflow", () => {
    test("should display empty state with all weapons at level 0", async ({
      page,
    }) => {
      // Arrange
      ...
      await expect(page.locator("#equipped-weapon-attack-pct")).toHaveText(
        "0%",
      );
    ...
    test("setting first weapon level updates inventory and summary stats", async ({
      page,
    }) => {
      // Arrange
      await page.goto(`${BASE_URL}/#/setup/weapon-levels`);
    ...
    test("clicking star rating updates visual state and enforces max level", async ({
      page,
    }) => {
      // Arrange
      await page.goto(`${BASE_URL}/#/setup/weapon-levels`);
    ...
    test("equipping weapon shows equipped display and hides others", async ({
      page,
    }) => {
      // Arrange
      await page.goto(`${BASE_URL}/#/setup/weapon-levels`);
    ...
    test("switching equipped weapon unchecks previous", async ({ page }) => {
      ...
      await expect(
        page.locator("#equipped-checkbox-normal-t4"),
      ).not.toBeChecked();

      // Assert - Side effects: previous equipped display is hidden
      await expect(
        page.locator("#equipped-display-normal-t4"),
      ).not.toBeVisible();

      // Assert - Side effects: previous card doesn't have equipped class
      await expect(page.locator("#weapon-normal-t4")).not.toHaveClass(
        /equipped/,
      );
    ...
    test("star toggle behavior: clicking same star sets to 0", async ({
      page,
    }) => {
      // Arrange
      await page.goto(`${BASE_URL}/#/setup/weapon-levels`);
  ...
  test.describe("Adjustment Workflow", () => {
    test("changing existing weapon level recalculates displays", async ({
      page,
    }) => {
      // Arrange
      await applyWeaponLevelsFixture(page, WEAPON_LEVELS_EARLY);
      const initialInventory = await page
        .locator("#total-inventory-attack")
        .textContent();
    ...
    test("upgrade gain text shows when below max level", async ({ page }) => {
      ...
      const upgradeGainContainer = page.locator(
        "#upgrade-gain-container-normal-t4",
      );
  ...
  test.describe("Sub-Tab Switching", () => {
    test("switching from weapons grid to upgrade priority", async ({
      page,
    }) => {
      // Arrange
      await page.goto(`${BASE_URL}/#/setup/weapon-levels`);
    ...
    test("switching from upgrade priority to weapons grid", async ({
      page,
    }) => {
      // Arrange
      await page.goto(`${BASE_URL}/#/setup/weapon-levels`);
  ...
  test.describe("Cross-Tab Navigation Persistence", () => {
    test("navigating to equipment and back preserves weapon data", async ({
      page,
    }) => {
      ...
      const initialTotal = await page
        .locator("#total-inventory-attack")
        .textContent();
      const initialEquipped = await page
        .locator("#equipped-weapon-attack-pct")
        .textContent();
      ...
      await expect(page.locator("#total-inventory-attack")).toHaveText(
        initialTotal,
      );
      await expect(page.locator("#equipped-weapon-attack-pct")).toHaveText(
        initialEquipped,
    ...
    test("navigating to gear lab and back preserves weapon data", async ({
      page,
    }) => {
      // Arrange - Configure weapon levels
    ...
    test("navigating to statHub and back preserves weapon data", async ({
      page,
    }) => {
      // Arrange - Configure weapon levels
      await applyWeaponLevelsFixture(page, WEAPON_LEVELS_EARLY);
    ...
    test("full workflow across multiple tabs preserves all data", async ({
      page,
    }) => {
      // Arrange - Configure weapon levels
      await applyWeaponLevelsFixture(page, WEAPON_LEVELS_MID);
  ...
  test.describe("Page Reload Persistence", () => {
    ...
    test("reloading page with equipped weapon maintains equipped state", async ({
      page,
    }) => {
      // Arrange
      await applyWeaponLevelsFixture(page, WEAPON_LEVELS_EARLY);
