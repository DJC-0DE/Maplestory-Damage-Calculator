...
test.describe("Navigation Tests", () => {
    ...
    for (const [pageKey, pageData] of Object.entries(navigationStructure)) {
      test(`should navigate to ${pageData.title} page`, async ({ page }) => {
        ...
        await expect(page).toHaveURL(
          new RegExp(`/#/${pageKey}(/${pageData.defaultTab})?`),
        );
  ...
  test.describe("Default Tab Navigation", () => {
    test("should load page correctly when navigating to main page without tab", async ({
      page,
    }) => {
  ...
  test.describe("Cross-Page Navigation", () => {
    test("should navigate from Setup to Optimization to Predictions", async ({
      page,
    }) => {
      // Start on Setup
      await page.goto(`${BASE_URL}/#/setup/base-stats`);
  ...
  test.describe("Direct URL Navigation", () => {
    test("should load correct page when typing URL directly", async ({
      page,
    }) => {
      ...
      const tabElement = page
        .locator(
          '[data-tab="artifact-potential"], button:has-text("Artifact Potential")',
        )
        .first();
  ...
  test.describe("URL Update on Tab Switch", () => {
    test("should update URL when switching tabs via UI", async ({ page }) => {
      ...
      const innerAbilityButton = page
        .locator('button:has-text("Inner Ability")')
        .first();
