...
test.describe("Base Stats - Character Setup Workflow", () => {
  ...
  test("new user selects hero class and configures basic stats", async ({
    page,
  }) => {
  ...
  test("user sets character level and job tier", async ({ page }) => {
    ...
    await expect(page.locator(JOB_TIER_BUTTONS["3rd"])).not.toHaveClass(
      /active/,
    );
  ...
  test("user inputs main stat values for warrior class", async ({ page }) => {
    ...
    const primaryStat = await page.evaluate(
      () => document.getElementById("primary-main-stat-base").value,
    );
    const secondaryStat = await page.evaluate(
      () => document.getElementById("secondary-main-stat-base").value,
    );
...
test.describe("Base Stats - Class Switching Workflow", () => {
  ...
  test("switching from warrior to mage updates stat rows and preserves inputs", async ({
    page,
  }) => {
  ...
  test("switching from warrior to dark knight shows defense input", async ({
    page,
  }) => {
    // Arrange
    await page.click(CLASS_SELECTORS.hero);
    ...
    // Assert - Class changed
    await expect(page.locator(CLASS_SELECTORS.darkKnight)).toHaveClass(
      /selected/,
    );
  ...
  test("switching from archer to thief updates stat rows correctly", async ({
    page,
  }) => {
    // Arrange
  ...
  test("switching classes maintains configured values where applicable", async ({
    page,
  }) => {
    ...
    await expect(page.locator(CLASS_SELECTORS.marksman)).toHaveClass(
      /selected/,
    );
    await expect(page.locator(STAT_ROWS.dex)).toBeVisible();
    await expect(page.locator(STAT_ROWS.str)).toBeVisible();
  ...
  test("switching from 4th job to 3rd job updates mastery tables", async ({
    page,
  }) => {
    ...
    await expect(page.locator(JOB_TIER_BUTTONS["3rd"])).toHaveClass(/active/);
    await expect(page.locator(JOB_TIER_BUTTONS["4th"])).not.toHaveClass(
      /active/,
    );
...
test.describe("Base Stats - Configuration Adjustment Workflow", () => {
  ...
  test("user adjusts individual stat inputs and values persist", async ({
    page,
  }) => {
    // Arrange
    await applyBaseStatsFixture(page, HERO_LEVEL_60);
  ...
  test("user adjusts skill levels and skill coefficient updates", async ({
    page,
  }) => {
    // Arrange
    ...
    const skillCoeff = await page.evaluate(
      () => document.getElementById("skill-coeff-base").value,
    );
...
test.describe("Base Stats - Cross-Tab Integration", () => {
  ...
  for (const testCase of tabNavigationTests) {
    test(`base stats persist when navigating to ${testCase.name} tab`, async ({
      page,
    }) => {
      ...
      if (testCase.assertions) {
        ...
        await expect(page.locator("#character-level")).toHaveValue(
          String(testCase.fixture.level),
        );
        await expect(page.locator(STAT_INPUTS.attack)).toHaveValue(
          String(testCase.fixture.attack),
        );
        await expect(
          page.locator(CLASS_SELECTORS[testCase.fixture.class]),
  ...
  test("base stats persist across multiple tab navigations", async ({
    page,
  }) => {
    // Arrange
    await applyBaseStatsFixture(page, BOWMASTER_LEVEL_80);
    ...
    await expect(page.locator(CLASS_SELECTORS.bowmaster)).toHaveClass(
      /selected/,
    );
    await expect(page.locator(JOB_TIER_BUTTONS["3rd"])).toHaveClass(/active/);
  ...
  test("changing class in base stats affects other tabs", async ({ page }) => {
    ...
    await expect(page.locator(CLASS_SELECTORS.shadower)).toHaveClass(
      /selected/,
    );
