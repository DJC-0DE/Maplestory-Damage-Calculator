...
import {
  ...
  refreshPresetsUI,
} from "@ui/companions-presets-ui.js";
...
function formatEffects(effects) {
  ...
  return processEffects(effects)
    .map(
      (entry) =>
        `${entry.displayName}: +${entry.value}${entry.isPercentage ? "%" : ""}`,
    )
    .join("<br>");
}
  ...
  if (savedCompanion) {
    setTimeout(() => {
      const icon = document.querySelector(
        `.companion-icon[data-companion="${savedCompanion.companionKey}"]`,
      );
      if (icon) {
...
window.handleLevelChange = function (companionKey, newLevel) {
  ...
  const levelDisplay = document.querySelector(
    `.companion-icon-level[data-companion="${companionKey}"]`,
  );
  if (levelDisplay) {
...
function updateSummary() {
  ...
  if (Object.keys(totalEffects).length === 0) {
    container.innerHTML =
      '<span style="color: var(--text-secondary); font-style: italic;">No unlocked companions</span>';
    return;
  }
