...
} from "@ui/comparison-ui.js";
import {
  equippedStatCount,
  setEquippedStatCount,
  availableStats,
  allItemStatProperties,
} from "@core/constants.js";
import {
  equipItemFromComparison,
  getCurrentSlot,
...
export function unequipItem() {
  ...
  const hasStats = allItemStatProperties.some(
    (prop) => equippedItem[prop] !== 0,
  );
  ...
  statDamageBase.value = (
    parseFloat(statDamageBase.value) - statDamageChange
  ).toFixed(1);

  const critRateBase = document.getElementById("crit-rate-base");
  critRateBase.value = (
    parseFloat(critRateBase.value) - equippedItem.critRate
  ).toFixed(1);

  const critDamageBase = document.getElementById("crit-damage-base");
  critDamageBase.value = (
    parseFloat(critDamageBase.value) - equippedItem.critDamage
  ).toFixed(1);
  ...
  ["1st", "2nd", "3rd", "4th"].forEach((tier, index) => {
    ...
    if (skillLevelInput && equippedItem[skillKey]) {
      skillLevelInput.value = Math.max(
        0,
        parseInt(skillLevelInput.value || 0) - equippedItem[skillKey],
      );
    }
  });
  ...
  normalDamageBase.value = (
    parseFloat(normalDamageBase.value) - equippedItem.normalDamage
  ).toFixed(1);

  const bossDamageBase = document.getElementById("boss-damage-base");
  bossDamageBase.value = (
    parseFloat(bossDamageBase.value) - equippedItem.bossDamage
  ).toFixed(1);

  const damageBase = document.getElementById("damage-base");
  damageBase.value = (
    parseFloat(damageBase.value) - equippedItem.damage
  ).toFixed(1);

  const minDamageBase = document.getElementById("min-damage-base");
  minDamageBase.value = (
    parseFloat(minDamageBase.value) - equippedItem.minDamage
  ).toFixed(1);

  const maxDamageBase = document.getElementById("max-damage-base");
  maxDamageBase.value = (
    parseFloat(maxDamageBase.value) - equippedItem.maxDamage
  ).toFixed(1);
  ...
  const newItemId = document.querySelectorAll(
    `[id^="comparison-tab-${currentSlot}-"]`,
  ).length;

  document.getElementById(`item-${currentSlot}-${newItemId}-name`).value =
    equippedItem.name || "Unequipped Item";

  // Get base attack from the attack field (not from stat lines)
  const baseAttack =
    parseFloat(document.getElementById("equipped-attack")?.value) || 0;
  document.getElementById(`item-${currentSlot}-${newItemId}-attack`).value =
    baseAttack;
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    if (typeElem && valueElem && typeElem.value && valueElem.value) {
      ...
      const container = document.getElementById(
        `item-${currentSlot}-${newItemId}-stats-container`,
      );
      const statCount = container.children.length;
      document.getElementById(
        `item-${currentSlot}-${newItemId}-stat-${statCount}-type`,
      ).value = typeElem.value;
      document.getElementById(
        `item-${currentSlot}-${newItemId}-stat-${statCount}-value`,
...
export function addEquippedStat() {
  ...
  statDiv.style.cssText =
    "display: grid; grid-template-columns: 1fr 80px auto; gap: 8px; margin-bottom: 6px; align-items: end;";
...
export function initializeEquipmentSlots() {
  ...
  setTimeout(() => {
    slots.forEach((slot) => {
      ...
      if (attackInput)
        attackInput.addEventListener("input", saveEquipmentSlots);
      if (mainStatInput)
        mainStatInput.addEventListener("input", saveEquipmentSlots);
      if (damageAmpInput)
        damageAmpInput.addEventListener("input", saveEquipmentSlots);
    });
  }, 0);
}
