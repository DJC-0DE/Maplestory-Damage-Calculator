...
} from "@core/companions/index.js";
import {
  addStat,
  subtractStat,
  Stat,
} from "@core/services/stat-inputs-service.js";
import {
  calculateBothDpsDifferences,
  presetHasAnyCompanion,
  generateOptimalPreset,
} from "@core/companions/companion-logic.js";
...
function renderPresetsPanel() {
  ...
  const maxBossDpsPreset = generateOptimalPreset(
    ...
    getCompanion,
    getMaxCompanionLevel,
    getLockedMainCompanion("optimal-boss"),
  );
  const maxNormalDpsPreset = generateOptimalPreset(
    ...
    getCompanion,
    getMaxCompanionLevel,
    getLockedMainCompanion("optimal-normal"),
  );
...
function renderPresetRow(
  ...
  const borderColor = isSpecialPreset
    ? "#fbbf24"
    : isEquipped
      ? "#10b981"
      : "var(--border-color)";
        ...
        ">
            ...
            ">
                ...
                ${presetData.subs
                  .map((sub, index) =>
                    isSpecialPreset
                      ? renderReadOnlySlot(sub, 50)
                      : renderSlot(presetId, "sub", index, sub, 50),
                  )
                  .join("")}
  ...
  if (showDpsComparison && presetHasAnyCompanion(presetData)) {
    ...
    const dpsResults = calculateBothDpsDifferences(
      currentPresetEffects,
      targetPresetEffects,
    );
    ...
    if (isEquipped) {
      ...
      const bossPctGain =
        dpsResults.boss.baselineDps > 0
          ? (bossGainOverBaseline / dpsResults.boss.baselineDps) * 100
          : 0;
      const normalPctGain =
        dpsResults.normal.baselineDps > 0
          ? (normalGainOverBaseline / dpsResults.normal.baselineDps) * 100
          : 0;
...
function renderSlot(presetId, slotType, slotIndex, companionKey, size) {
  ...
  const iconName = isMain
    ? getClassWebpName(className)
    : getClassPngName(className);
...
function handleSlotClick(slot) {
  ...
  const clearBtnContainer = document.getElementById(
    "clear-slot-selection-container",
  );
...
function showSelectionFeedback(presetId, mainRarity) {
  ...
  const slotElement = document.querySelector(
    `[data-slot-id="${presetId}-${slot.type}-${slot.index}"]`,
  );
...
export function assignCompanionToSlot(companionKey) {
  ...
  // Check if this is an optimal preset slot
  const isOptimalPreset =
    slotInfo.presetId === "optimal-boss" ||
    slotInfo.presetId === "optimal-normal";
...
function createStatComparisonTable(currentEffects, newEffects) {
  ...
  for (const stat of allStats) {
    ...
    const isPercentage = !["hitChance", "maxHp", "attack", "mainStat"].includes(
      stat,
    );
    ...
    const currentClass =
      currentValue > 0 ? "stat-value-negative" : "stat-value-neutral";
    // Determine color class for new value (being added)
    const newClass =
      newValue > 0 ? "stat-value-positive" : "stat-value-neutral";
...
function showEquipConfirmModal(presetId, currentEffects, newEffects) {
  return new Promise((resolve) => {
    ...
    message.textContent =
      "Are the stats from the currently equipped preset already incorporated in your input stats?";

    // Create stat comparison table
    const tableContainer = document.createElement("div");
    tableContainer.className = "stat-comparison-table";
    tableContainer.innerHTML = createStatComparisonTable(
      currentEffects,
      newEffects,
    );
...
window.equipPreset = async function (presetId) {
  ...
  const hasEquipEffects =
    Object.keys(newPresetEffects).length > 0 ||
    Object.keys(currentPresetEffects).length > 0;

  if (hasEquipEffects) {
    // Show confirmation modal
    const userChoice = await showEquipConfirmModal(
      presetId,
      currentPresetEffects,
      newPresetEffects,
    );
  ...
  if (currentLock) {
    ...
    setTimeout(() => {
      ...
      const presetRow = document.querySelector(
        `.preset-row[data-preset="${optimalType}"]`,
      );
      if (presetRow) {
        // Make all slots in this preset row glow (main + readonly subs)
        const allSlots = presetRow.querySelectorAll(
          ".optimal-main-slot, .optimal-readonly-slot",
        );
