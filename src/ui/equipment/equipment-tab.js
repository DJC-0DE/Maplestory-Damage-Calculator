...
function saveSlotData(slotId) {
  ...
  localStorage.setItem(
    `equipped.${slotId}`,
    JSON.stringify(equipmentData[slotId]),
  );
}
...
function loadEquipmentData() {
  EQUIPMENT_SLOTS.forEach((slot) => {
    ...
    if (saved) {
      ...
      const statsContainer = document.getElementById(
        `equipment-${slot.id}-stats-container`,
      );
    ...
    if (data.statLines && Array.isArray(data.statLines)) {
      data.statLines.forEach((statLine) => {
        ...
        const typeInput = document.getElementById(
          `equipment-${slotId}-stat-${statCount}-type`,
        );
        const valueInput = document.getElementById(
          `equipment-${slotId}-stat-${statCount}-value`,
        );
