...
export function updateScrollLevelInfo() {
  const level =
    document.querySelector('input[name="scroll-level"]:checked')?.value || "65";
  ...
  const bonusDisplay =
    enhancementBonus > 0
      ? `<br><strong style="color: var(--accent-success);">Enhancement Bonus: +${(enhancementBonus * 100).toFixed(0)}% success rate</strong>`
      : "";

  if (level === "65") {
    const l65_70_rate = (
      (SCROLLS_L65.L65_70.baseSuccess + enhancementBonus) *
      100
    ).toFixed(0);
    const l65_30_rate = (
      (SCROLLS_L65.L65_30.baseSuccess + enhancementBonus) *
      100
    ).toFixed(0);
    ...
    const l85_70_rate = (
      (SCROLLS_L85.L85_70.baseSuccess + enhancementBonus) *
      100
    ).toFixed(0);
    const l85_30_rate = (
      (SCROLLS_L85.L85_30.baseSuccess + enhancementBonus) *
      100
    ).toFixed(0);
    const l85_15_rate = (
      (SCROLLS_L85.L85_15.baseSuccess + enhancementBonus) *
      100
    ).toFixed(0);
...
export function attemptScrollRun(strategy, budget, traceUsed, scrolls) {
  ...
  for (let slot = 1; slot <= NUM_SLOTS; slot++) {
    const scrollType = strategy.selectScroll(
      slot,
      successfulSlots,
      failedSlots,
      traceUsed + attemptCost,
      budget,
    );
    ...
    if (strategy.shouldResetEarly) {
      const shouldResetEarly = strategy.shouldResetEarly(
        slotResults,
        slot,
        successfulSlots,
        failedSlots,
...
export function createL65Strategies() {
  ...
  strategies.push({
    ...
    description:
      "Reset until 30% succeeds on slot 1. Then 30% on slots 5&10, 70% on others. Low budget efficient.",
    selectScroll: (slot) => {
  ...
  strategies.push({
    ...
    description:
      "70% L65 on all slots except 5&10, which use 30% (42% and 52% success rates). No early resets.",
    selectScroll: (slot) => {
      if (slot === 5 || slot === 10) return "L65_30";
      return "L65_70";
    },
  ...
  strategies.push({
    ...
    description:
      "Reset until 30% succeeds on slot 10 (52% with +20% bonus). Then 30% on slots 1&5, 70% on others.",
    selectScroll: (slot) => {
  ...
  strategies.push({
    ...
    description:
      "Reset until 30% succeeds on slot 5 (42% with +10% bonus). Then 30% on slots 1&10, 70% on others.",
    selectScroll: (slot) => {
  ...
  strategies.push({
    ...
    description:
      "Reset until slot 1 succeeds, then reset until slot 10 succeeds (both 30%). Then 30% on slot 5, 70% on others. High damage floor.",
    selectScroll: (slot) => {
      if (slot === 1 || slot === 10) return "L65_30";
      if (slot === 5) return "L65_30";
      return "L65_70";
  ...
  strategies.push({
    ...
    description:
      "Reset until slot 1 succeeds, then reset until slot 5 succeeds (both 30%). Then 30% on slot 10, 70% on others. Medium budget.",
    selectScroll: (slot) => {
      if (slot === 1 || slot === 5) return "L65_30";
      if (slot === 10) return "L65_30";
      return "L65_70";
  ...
  strategies.push({
    ...
    description:
      "Reset until slots 1, 5, and 10 all succeed sequentially (all 30%). 70% on others. High budget, guarantees 3x 30% success.",
    selectScroll: (slot) => {
  ...
  strategies.push({
    ...
    description:
      "Reset until slot 1 succeeds with 30%, then use 30% L65 for ALL remaining slots. High damage amp potential, high budget required.",
    selectScroll: () => "L65_30",
    shouldResetEarly: (slotResults, currentSlot) => {
      if (currentSlot === 1 && slotResults.length >= 1) {
        const slot1 = slotResults[0];
  ...
  strategies.push({
    ...
    description:
      "Reset until slot 1 succeeds with 30%, then 30% on slots 5&10, 70% elsewhere. Also reset if total damage amp < 0.5%. High budget.",
    selectScroll: (slot) => {
      if (slot === 1) return "L65_30";
      if (slot === 5 || slot === 10) return "L65_30";
      return "L65_70";
  ...
  strategies.push({
    ...
    description:
      "Reset until slot 1 succeeds with 30%, then 30% on slots 5&10, 70% elsewhere. Also reset if both bonus slots (5&10) fail. Medium budget.",
    selectScroll: (slot) => {
      if (slot === 1) return "L65_30";
      if (slot === 5 || slot === 10) return "L65_30";
      return "L65_70";
  ...
  strategies.push({
    ...
    description:
      "Reset until slot 1 succeeds with 30%, then 30% on slots 5&10, 70% elsewhere. Also reset if <8 total successes. Very high budget.",
    selectScroll: (slot) => {
      if (slot === 1) return "L65_30";
      if (slot === 5 || slot === 10) return "L65_30";
      return "L65_70";
...
export function createL85Strategies() {
  ...
  strategies.push({
    ...
    description:
      "15% L85 on slot 1, reset until success. Then 15% on slots 5&10, 70% on others. No resets after slot 1 succeeds.",
    selectScroll: (slot) => {
  ...
  strategies.push({
    ...
    description:
      "Lock slot 1 with 15%, then use 30% L85 for all remaining slots. Higher risk/reward for high budgets.",
    selectScroll: (slot) => {
      if (slot === 1) return "L85_15";
      return "L85_30";
    },
  ...
  strategies.push({
    ...
    description:
      "Lock slot 1, then 15% on slots 5&10, 70% elsewhere. Reset if <7 total successes. Very high budget.",
    selectScroll: (slot) => {
      if (slot === 1) return "L85_15";
      if (slot === 5 || slot === 10) return "L85_15";
      return "L85_70";
  ...
  strategies.push({
    ...
    description:
      "Reset until slot 1 succeeds with a 15%, 70% until slot 5, reset until slot 15% suceeds here too, 70% until slot 10 where finish off with 15%.",
    selectScroll: (slot) => {
  ...
  strategies.push({
    ...
    description:
      "Lock slot 1, then use 15% L85 for ALL remaining slots. Ultra-aggressive, extreme budget required.",
    selectScroll: () => "L85_15",
    shouldResetEarly: (slotResults, currentSlot) => {
      if (currentSlot === 1 && slotResults.length >= 1) {
        const slot1 = slotResults[0];
...
export async function runScrollSimulation() {
  const budget = parseInt(
    document.getElementById("scroll-spell-trace-budget").value,
  );
  const numSimulations = parseInt(
    document.getElementById("scroll-simulations").value,
  );
  const scrollLevel =
    document.querySelector('input[name="scroll-level"]:checked')?.value || "65";
  ...
  const strategies =
    scrollLevel === "65" ? createL65Strategies() : createL85Strategies();
  const results = {};
  ...
  for (const strategyId in results) {
    ...
    data.avgDamageAmp =
      data.damageAmps.reduce((a, b) => a + b, 0) / numSimulations;
    data.avgSuccess =
      data.successCounts.reduce((a, b) => a + b, 0) / numSimulations;
    data.avgResets =
      data.resetCounts.reduce((a, b) => a + b, 0) / numSimulations;
    data.avgTraceUsed =
      data.traceUsed.reduce((a, b) => a + b, 0) / numSimulations;

    // Calculate damage gain
    const damageGain = calculateScrollDamageGain(
      data.avgAttack,
      data.avgDamageAmp,
    );
...
export function displayScrollResults(results, budget, numSimulations) {
  ...
  const sortedStrategies = Object.entries(results).sort(
    (a, b) => b[1].dpsGain - a[1].dpsGain,
  );
                    ...
                    ${(() => {
                      ...
                      bins.forEach((count, index) => {
                        if (count > 0) {
                          ...
                          const barWidth = ((count / maxCount) * 100).toFixed(
                            1,
                          );
                          ...
                          histogramHTML +=
                            ...
                            '<div style="background: linear-gradient(90deg, #007AFF, #34C759); width: ' +
                            barWidth +
...
export function switchScrollStrategyTab(strategyId) {
  ...
  const selectedContent = document.getElementById(
    `scroll-strategy-tab-${strategyId}`,
  );
  if (selectedContent) {
