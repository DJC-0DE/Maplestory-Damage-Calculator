...
function getFactorForLevel(level, factorIndex) {
  ...
  const levels = Object.keys(SKILL_LEVEL_FACTOR_TABLE)
    .map(Number)
    .sort((a, b) => a - b);
 ...
 */
function calculateSkillInputLevel(
  characterLevel,
  jobTier,
  skillLevelBonus = 0,
) {
 ...
 */
export function calculate3rdJobSkillCoefficient(
  characterLevel,
  skillLevel = 0,
) {
 ...
 */
export function calculate4thJobSkillCoefficient(
  characterLevel,
  skillLevel = 0,
) {
...
export const DARK_KNIGHT_SKILLS = {
  ...
  CROSS_OVER_CHAINS: {
    ...
    effects: [
      ...
      "Toughness Bonus: {toughnessBonus}",
    ],
    isPassive: false,
    jobTier: "thirdJob",
...
export function calculateHyperBody(level) {
  ...
  return {
    attackBonus: calculateSkillValue(
      config.attackBonus.base,
      config.attackBonus.factorIndex,
      level,
    ),
    defenseBonus: calculateSkillValue(
      config.defenseBonus.base,
      config.defenseBonus.factorIndex,
      level,
    ),
  };
}
...
export function calculateFinalAttack(level) {
  ...
  return calculateSkillValue(
    config.baseDamage,
    config.factorIndex,
    level,
    config.scalesWithLevel,
  );
}
  ...
  return {
    ...
    thirdJobSkills: {
      ...
      crossOverChains: {
        ...
        toughnessBonus:
          calculateCrossOverChainsToughness(level).toFixed(2) + "%",
      },
...
export function calculateJobSkillPassiveGains(
  ...
  for (let i = 0; i < jobTiers.length; i++) {
    ...
    for (const { name, skillData } of passives) {
      ...
      const baseInputLevel = calculateSkillInputLevel(
        characterLevel,
        tierNumber,
        0,
      );
      const bonusInputLevel = calculateSkillInputLevel(
        characterLevel,
        tierNumber,
        tierBonus,
      );
    ...
    const complexPassivesForTier = getComplexPassivesByTier(
      classSkills,
      tierNumber,
    );
