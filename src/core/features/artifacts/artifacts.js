...
export function renderEquippedArtifacts() {
  const container = document.getElementById(
    "artifacts-equipped-panelrtifacts-equipped-panel",
  );
  if (!container) return;
  ...
  ["slot1", "slot2", "slot3"].forEach((slotKey, index) => {
    ...
    if (
      ...
      const libraryEntry = artifactLibrary[artifactName] || {
        stars: 0,
        potentials: [null, null, null],
      };
...
export function renderArtifactGrid() {
  ...
  const equippedNames = [preset.slot1, preset.slot2, preset.slot3].filter(
  ...
  artifactsData.forEach((artifact) => {
    ...
    const libraryEntry = artifactLibrary[artifact.name] || {
      stars: 0,
      potentials: [null, null, null],
    };
    ...
    html += `
            <div onclick="previewArtifact('${artifact.name.replace(/'/g, "\\'")}')"
                ...
                ${
                  isEquipped
                    ? `<div style="position: absolute; top: 4px; right: 4px; background: var(--accent-success);
...
export function saveArtifactsToLocalStorage() {
  try {
    ...
    localStorage.setItem(
      "currentArtifactPreset",
      currentArtifactPreset.toString(),
    );
  } catch (error) {
