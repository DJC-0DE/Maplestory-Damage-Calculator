...
export function calculateArtifactPotentialRankings() {
  ...
  Object.entries(artifactPotentialData).forEach(([rarity, stats]) => {
    Object.entries(stats).forEach(([statName, value]) => {
      ...
      const bossDPSPercentChange =
        baselineBossDamage.dps > 0
          ? (bossDPSGain / baselineBossDamage.dps) * 100
          : 0;
      const normalDPSPercentChange =
        baselineNormalDamage.dps > 0
          ? (normalDPSGain / baselineNormalDamage.dps) * 100
          : 0;
      const avgDPSPercentChange =
        (bossDPSPercentChange + normalDPSPercentChange) / 2;
...
export function renderArtifactPotential() {
  ...
  html +=
    '<div class="table-scrollable"><div class="artifact-table-container">';
  html += '<table class="table"><thead><tr>';
  html += "<th>Rank</th>";
  html += "<th>Rarity & Stat</th>";
  html += "<th>Value</th>";
  html +=
    '<th class="sortable" onclick="sortArtifactTable(3)">Boss DPS Gain</th>';
  html +=
    '<th class="sortable" onclick="sortArtifactTable(4)">Normal DPS Gain</th>';
  html +=
    '<th class="sortable" onclick="sortArtifactTable(5)">Avg DPS Gain</th>';
  html += "</tr></thead><tbody>";
