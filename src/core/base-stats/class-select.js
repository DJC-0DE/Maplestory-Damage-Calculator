...
import { setSelectedClass, setSelectedJobTier } from "@core/state/state.js";
  ...
  try {
    ...
    if (savedClass) {
      ...
      if (classElement) {
        ...
        const defenseInputGroup = document.getElementById(
          "defense-input-group",
        );
        if (defenseInputGroup) {
          defenseInputGroup.style.display =
            savedClass === "dark-knight" ? "flex" : "none";
        }
        ...
        if (savedClass === "hero" || savedClass === "dark-knight") {
          ...
          savedClass === "arch-mage-il" ||
          savedClass === "arch-mage-fp"
        ) {
        ...
        console.warn(
          `Class element 'class-${savedClass}' not found in DOM, but state was set`,
        );
      }
    } else {
