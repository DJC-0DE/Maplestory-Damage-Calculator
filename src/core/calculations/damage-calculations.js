...
export function calculateDamage(stats, monsterType) {
  ...
  const totalSkillMastery =
    stats.skillMastery + (monsterType === "boss" ? stats.skillMasteryBoss : 0);
  const baseDamage =
    stats.attack * (stats.skillCoeff / 100) * (1 + totalSkillMastery / 100);
  ...
  const monsterDamage =
    monsterType === "boss" ? stats.bossDamage : stats.normalDamage;
  ...
  const nonCritMin =
    baseHitDamage * (Math.min(stats.minDamage, stats.maxDamage) / 100);
...
export function calculateStatWeights(setup, stats) {
  ...
  html +=
    '<h3 style="margin: 0 0 12px 0; font-size: 0.9375rem; font-weight: 700; color: var(--text-primary); letter-spacing: -0.02em;">Flat Stats</h3>';
  html += '<div class="">';
  ...
  html +=
    '<h3 style="margin: 0 0 12px 0; font-size: 0.9375rem; font-weight: 700; color: var(--text-primary); letter-spacing: -0.02em;">Percentage Stats</h3>';
  html += '<div class="">';
  html += `<table class="table" id="stat-pred-table-${setup}-percentage">`;
  html +=
    '<thead><tr><th style="padding: 8px 10px; font-size: 0.75rem;">Stat</th>';
...
window.sortStatPredictions = function (
  setup,
  tabName,
  colIndex,
  headerElement,
  ...
  const rows = Array.from(tbody.querySelectorAll("tr")).filter(
    (row) => !row.classList.contains("chart-row"),
  );
...
export function calculateStatEquivalency(sourceStat) {
  ...
  const stats = {
    ...
    normalDamage: parseFloat(
      document.getElementById("normal-damage-base").value,
    ),
    skillCoeff: parseFloat(document.getElementById("skill-coeff-base").value),
    skillMastery: parseFloat(
      document.getElementById("skill-mastery-base").value,
    ),
    skillMasteryBoss: parseFloat(
      document.getElementById("skill-mastery-boss-base").value,
    ),
  ...
  const statMapping = {
    attack: {
      label: "Attack",
      getValue: () =>
        parseFloat(document.getElementById("equiv-attack").value) || 0,
    ...
    "main-stat": {
      label: "Main Stat",
      getValue: () =>
        parseFloat(document.getElementById("equiv-main-stat").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "main-stat-pct": {
      label: "Main Stat %",
      getValue: () =>
        parseFloat(document.getElementById("equiv-main-stat-pct").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "skill-coeff": {
      ...
      getValue: () =>
        parseFloat(document.getElementById("equiv-skill-coeff").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "skill-mastery": {
      label: "Skill Mastery",
      getValue: () =>
        parseFloat(document.getElementById("equiv-skill-mastery").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    damage: {
      label: "Damage",
      getValue: () =>
        parseFloat(document.getElementById("equiv-damage").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "final-damage": {
      label: "Final Damage",
      getValue: () =>
        parseFloat(document.getElementById("equiv-final-damage").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "boss-damage": {
      label: "Boss Damage",
      getValue: () =>
        parseFloat(document.getElementById("equiv-boss-damage").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "normal-damage": {
      ...
      getValue: () =>
        parseFloat(document.getElementById("equiv-normal-damage").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "stat-damage": {
      label: "Main Stat %",
      getValue: () =>
        parseFloat(document.getElementById("equiv-stat-damage").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "damage-amp": {
      ...
      getValue: () =>
        parseFloat(document.getElementById("equiv-damage-amp").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "min-damage": {
      ...
      getValue: () =>
        parseFloat(document.getElementById("equiv-min-damage").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "max-damage": {
      ...
      getValue: () =>
        parseFloat(document.getElementById("equiv-max-damage").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "crit-rate": {
      label: "Critical Rate",
      getValue: () =>
        parseFloat(document.getElementById("equiv-crit-rate").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "crit-damage": {
      ...
      getValue: () =>
        parseFloat(document.getElementById("equiv-crit-damage").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "attack-speed": {
      label: "Attack Speed",
      getValue: () =>
        parseFloat(document.getElementById("equiv-attack-speed").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
    ...
    "def-pen": {
      ...
      getValue: () =>
        parseFloat(document.getElementById("equiv-def-pen").value) || 0,
      applyToStats: (stats, value) => {
        const service = new StatCalculationService(stats);
  ...
  const modifiedStats = statMapping[sourceStat].applyToStats(
    stats,
    sourceValue,
  );
  ...
  let html =
    '<div style="background: linear-gradient(135deg, rgba(0, 122, 255, 0.08), rgba(88, 86, 214, 0.05)); border: 2px solid rgba(0, 122, 255, 0.2); border-radius: 16px; padding: 25px; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.1);">';
  ...
  Object.entries(statMapping).forEach(([statId, statConfig]) => {
    ...
    if (maxValue === null) {
      ...
      maxValue =
        statId === "attack" ? 100000 : statId === "main-stat" ? 50000 : 1000;
    }
    ...
    if (statConfig.isMultiplicative) {
      icon =
        ' <span style="font-size: 0.9em; opacity: 0.7;" title="Multiplicative stat">ℹ️</span>';
