...
import { initializeCubePotential } from "@core/cube/cube-potential.js";
...
} from "@ui/presets-ui.js";
import {
  initializeWeapons,
  updateWeaponBonuses,
} from "@core/weapon-levels/weapons-ui.js";
import {
  initializeEquipmentTab,
  migrateLegacyData,
} from "@ui/equipment/equipment-tab.js";
import {
  initializeSlotComparison,
  getCurrentSlot,
} from "@ui/comparison/slot-comparison.js";
...
import {
  ...
  selectJobTier,
  selectMasteryTab,
} from "./base-stats/class-select.js";
...
export function calculate() {
  ...
  resultsHTML += displayResults(
    equippedItem.name || "Currently Equipped",
    baseStats,
    "equipped",
    true,
    null,
  ...
  if (tabsContainer) {
    const slotTabs = tabsContainer.querySelectorAll(
      `[id^="comparison-tab-${currentSlot}-"]`,
    );
  ...
  comparisonItems.forEach((item) => {
    ...
    const context = {
      ...
      characterLevel:
        parseInt(document.getElementById("character-level")?.value) || 0,
      jobTier: getSelectedJobTier(),
      baseSkillLevel1st:
        parseInt(document.getElementById("skill-level-1st-base")?.value) || 0,
      baseSkillLevel2nd:
        parseInt(document.getElementById("skill-level-2nd-base")?.value) || 0,
      baseSkillLevel3rd:
        parseInt(document.getElementById("skill-level-3rd-base")?.value) || 0,
      baseSkillLevel4th:
        parseInt(document.getElementById("skill-level-4th-base")?.value) || 0,
  ...
  document.getElementById("results-container").innerHTML =
    resultsHTML ||
    '<p style="text-align: center; color: #b3d9ff;">Add comparison items to see results</p>';
...
export function switchBaseStatsSubTab(subTabName) {
  ...
  const buttons = document.querySelectorAll(
    "#setup-base-stats .tab-button, #setup-base-stats .optimization-sub-tab-button",
  );
...
const SKILL_DATA = (() => {
  ...
  return {
    skills: {
      secondJob: allSkills.filter(
        (s) => !s.isPassive && s.jobTier === "secondJob",
      ),
      thirdJob: allSkills.filter(
        (s) => !s.isPassive && s.jobTier === "thirdJob",
      ),
    },
    passives: {
      secondJob: allSkills.filter(
...
function populateSkillDetails() {
  ...
  const baseInputLevel3rd = Math.max(
    0,
    Math.min((characterLevel - 60) * 3, 120),
  );
  ...
  if (skillsGrid2nd) {
    skillsGrid2nd.innerHTML = SKILL_DATA.skills.secondJob
      .map((skill) => renderSkillCard(skill, "skills", "secondJob"))
      .join("");
  }
  ...
  if (skillsGrid3rd) {
    skillsGrid3rd.innerHTML = SKILL_DATA.skills.thirdJob
      .map((skill) => renderSkillCard(skill, "skills", "thirdJob"))
      .join("");
  }
  ...
  if (passivesGrid2nd) {
    passivesGrid2nd.innerHTML = SKILL_DATA.passives.secondJob
      .map((skill) => renderSkillCard(skill, "passives", "secondJob"))
      .join("");
  }
  ...
  if (passivesGrid3rd) {
    passivesGrid3rd.innerHTML = SKILL_DATA.passives.thirdJob
      .map((skill) => renderSkillCard(skill, "passives", "thirdJob"))
      .join("");
  }
}
...
export function showSkillDescription(skillKey, category, jobTier) {
  ...
  const effectiveLevel =
    jobTier === "secondJob" ? effectiveLevel2nd : effectiveLevel3rd;
  const baseInputLevel =
    jobTier === "secondJob" ? baseInputLevel2nd : baseInputLevel3rd;
  ...
  const effectsHTML = skillData.effects
    .map((effect) => {
...
window.onload = function () {
  ...
  if (loaded) {
    ...
    if (contentTypeData && contentTypeData.contentType) {
      ...
      if (
        contentTypeData.subcategory &&
        (contentTypeData.contentType === "stageHunt" ||
          contentTypeData.contentType === "growthDungeon")
      ) {
  ...
  pasteArea.addEventListener("paste", async (event) => {
    ...
    try {
      ...
      for (const parsedStat of parsedStats) {
        ...
        if (inputElement) {
          ...
          const primaryInput = document.getElementById(
            "primary-main-stat-base",
          );
          const secondaryInput = document.getElementById(
            "secondary-main-stat-base",
          );
      ...
      if (parsedStats.length > 0) {
        showToast(
          `Parsing successful! ${parsedStats.length} stats updated`,
          true,
        );
      } else {
        showToast(
          "Parsing failed! Make sure you are ONLY screenshotting the stats rows from the Character page and nothing else",
          false,
        );
      }
