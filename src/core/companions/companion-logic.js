...
function applyEffectsToService(service, effects, isRemoving = false) {
  ...
  const statTypes = {
    ...
    mainStat: (value) =>
      service.addMainStat(isRemoving ? -Math.abs(value) : value),

    // Main stat % with diminishing returns
    statDamage: (value) =>
      service.addMainStatPct(isRemoving ? -Math.abs(value) : value),
...
export function calculateDpsDifference(
  ...
  const dpsGain =
    baselineDps > 0
      ? ((newPresetGain - currentPresetGain) / baselineDps) * 100
      : 0;
...
export function calculateBothDpsDifferences(currentEffects, newEffects) {
  const bossResults = calculateDpsDifference(
    currentEffects,
    newEffects,
    "boss",
  );
  const normalResults = calculateDpsDifference(
    currentEffects,
    newEffects,
    "normal",
  );
...
export function generateOptimalPreset(
  ...
  if (lockedMainCompanion) {
    ...
    const lockedMainData = unlockedCompanions.find(
      (c) => c.companionKey === lockedMainCompanion,
    );
    if (!lockedMainData) {
    ...
    const subCandidates = unlockedCompanions.filter(
      (c) => c.companionKey !== lockedMainCompanion,
    );
    ...
    for (const subCombination of generateCombinations(subCandidates, 6)) {
      ...
      if (combinationsTested > maxCombinationsToTest) {
        console.warn(
          `Reached max combinations limit (${maxCombinationsToTest}), using best found so far`,
        );
        break;
      }
