...
export function switchPotentialType(type) {
  ...
  document
    .getElementById("cube-regular-potential-btn")
    .classList.toggle("active", type === "regular");
  document
    .getElementById("cube-bonus-potential-btn")
    .classList.toggle("active", type === "bonus");
...
export function selectCubeSlot(slotId) {
  ...
  document.querySelectorAll(".cube-slot-btn").forEach((btn) => {
    ...
    if (isActive) {
      const slotRarity =
        cubeSlotData[slotId]?.[currentPotentialType]?.rarity || "normal";
      const rarityColor = getRarityColor(slotRarity);
      btn.style.boxShadow = `0 4px 16px ${rarityColor}60, 0 0 0 2px ${rarityColor}`;
    } else {
      const slotRarity =
        cubeSlotData[btn.dataset.slot]?.[currentPotentialType]?.rarity ||
        "normal";
      const rarityColor = getRarityColor(slotRarity);
  ...
  const rankingsRaritySelector = document.getElementById(
    "cube-rankings-rarity-selector",
  );
  if (rankingsRaritySelector) {
    rankingsRaritySelector.value =
      cubeSlotData[currentCubeSlot][currentPotentialType].rarity;
  }
