...
} from "@core/features/inner-ability/inner-ability.js";
...
import {
  ...
  setShowPresetDpsComparison,
  getLockedMainCompanion,
  setLockedMainCompanion,
  updateAllContributions,
  updateCompanionEquippedContributions,
  getUnlockableStatsState,
  setUnlockableStatsState,
  getGuildBonusesState,
  setGuildBonusesState,
  getCubeSlotData,
  setCubeSlotData,
} from "./state.js";
...
export function saveToLocalStorage() {
  ...
  const contentTypeElements = document.querySelectorAll(
    ".content-type-selector.selected",
  );
  ...
  data.equippedItem.name =
    document.getElementById("equipped-name")?.value || "";
  data.equippedItem.attack =
  ...
  rarities.forEach((rarity) => {
    tiers.forEach((tier) => {
      ...
      const equippedCheckbox = document.getElementById(
...
export function loadFromLocalStorage() {
  ...
  try {
    ...
    if (data.baseSetup) {
      ...
      const characterLevelElement = document.getElementById("character-level");
      if (
        characterLevelElement &&
        data.baseSetup["character-level"] !== undefined
      ) {
    ...
    if (data.equippedItem) {
      document.getElementById("equipped-name").value =
        data.equippedItem.name || "";
      document.getElementById("equipped-attack").value =
        data.equippedItem.attack || "0";

      if (data.equippedItem.stats && data.equippedItem.stats.length > 0) {
        data.equippedItem.stats.forEach((stat) => {
          addEquippedStat();
          const typeElem = document.getElementById(
            `equipped-stat-${equippedStatCount}-type`,
          );
          const valueElem = document.getElementById(
            `equipped-stat-${equippedStatCount}-value`,
          );
      ...
      rarities.forEach((rarity) => {
        tiers.forEach((tier) => {
          ...
          if (weaponData) {
            ...
            // Set stars first
            if (starsInput) {
              const defaultStars = ["legendary", "mystic", "ancient"].includes(
                rarity,
              )
                ? 1
                : 5;
              const stars =
                weaponData.stars !== undefined
                  ? weaponData.stars
                  : defaultStars;
              ...
              for (let i = 1; i <= 5; i++) {
                const starElem = document.getElementById(
                  `star-${rarity}-${tier}-${i}`,
                );
                if (starElem) {
    ...
    if (data.equipmentSlots) {
      ...
      slotNames.forEach((slotId) => {
        if (data.equipmentSlots[slotId]) {
          const attackInput = document.getElementById(`slot-${slotId}-attack`);
          const mainStatInput = document.getElementById(
            `slot-${slotId}-main-stat`,
          );
          const damageAmpInput = document.getElementById(
            `slot-${slotId}-damage-amp`,
          );

          if (attackInput)
            attackInput.value = data.equipmentSlots[slotId].attack || 0;
          if (mainStatInput)
            mainStatInput.value = data.equipmentSlots[slotId].mainStat || 0;
          if (damageAmpInput)
            damageAmpInput.value = data.equipmentSlots[slotId].damageAmp || 0;
        }
      });
    }
    ...
    if (data.masteryBonuses) {
      ...
      if (data.masteryBonuses["3rd"] && data.masteryBonuses["4th"]) {
        ...
        if (data.masteryBonuses["3rd"].all) {
          [64, 68, 76, 80, 88, 92].forEach((level) => {
            const checkbox = document.getElementById(
              `mastery-3rd-all-${level}`,
            );
            if (
              checkbox &&
              data.masteryBonuses["3rd"].all[level] !== undefined
            ) {
        ...
        if (data.masteryBonuses["3rd"].boss) {
          [72, 84].forEach((level) => {
            const checkbox = document.getElementById(
              `mastery-3rd-boss-${level}`,
            );
            if (
              checkbox &&
              data.masteryBonuses["3rd"].boss[level] !== undefined
            ) {
        ...
        if (data.masteryBonuses["4th"].all) {
          [102, 106, 116, 120, 128, 132].forEach((level) => {
            const checkbox = document.getElementById(
              `mastery-4th-all-${level}`,
            );
            if (
              checkbox &&
              data.masteryBonuses["4th"].all[level] !== undefined
            ) {
        ...
        if (data.masteryBonuses["4th"].boss) {
          [111, 124].forEach((level) => {
            const checkbox = document.getElementById(
              `mastery-4th-boss-${level}`,
            );
            if (
              checkbox &&
              data.masteryBonuses["4th"].boss[level] !== undefined
            ) {
...
export function exportData() {
  ...
  navigator.clipboard
    ...
    .then(() => {
      alert(
        "✅ Data copied to clipboard! You can now paste it on another device.",
      );
    })
    .catch((err) => {
    ...
    if (comparisonItems && comparisonItems[slotId]) {
      try {
        ...
        if (!Array.isArray(items)) {
          console.warn(
            `Invalid comparison items format for ${slotId}: expected array`,
          );
          return;
        }
  ...
  navigator.clipboard
    ...
    .then((text) => {
      try {
        ...
        if (
          !confirm(
            "⚠️ This will overwrite your current data. Are you sure you want to continue?",
          )
        ) {
          return;
        }
        ...
        if (data.damageCalculatorData) {
          const dataString =
            typeof data.damageCalculatorData === "string"
              ? data.damageCalculatorData
        ...
        if (data.heroPowerPresets) {
          const dataString =
            typeof data.heroPowerPresets === "string"
              ? data.heroPowerPresets
        ...
        if (data.cubePotentialData) {
          const dataString =
            typeof data.cubePotentialData === "string"
              ? data.cubePotentialData
        ...
        alert(
          "❌ Failed to import data. Please make sure you copied valid data.",
        );
      }
    })
    .catch((err) => {
      ...
      alert(
        "❌ Failed to read clipboard. Please make sure you have data copied.",
      );
    });
}
...
export function attachSaveListeners() {
  ...
  if (equippedAttack)
    equippedAttack.addEventListener("input", saveToLocalStorage);
}
