 ...
 */
export function applyItemToStats(
  baseStats,
  equippedItem,
  comparisonItem,
  context,
  ...
  let mainStatChange =
    -(equippedItem.mainStat || 0) + (comparisonItem.mainStat || 0);
  ...
  const passiveGainsDiff =
    skillPassiveGainsService.calculatePassiveGainsDifference(
      equippedItem,
      comparisonItem,
      passiveGainsContext,
    );
  ...
  if (equippedFinalDamage !== 0 || comparisonFinalDamage !== 0) {
    ...
    const newMultiplier =
      (baseMultiplier / equippedMultiplier) * comparisonMultiplier;
