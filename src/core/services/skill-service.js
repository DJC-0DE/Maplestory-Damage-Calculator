...
export class SkillCoefficientService {
  ...
  calculateCoefficientDifference(equippedItem, comparisonItem, context) {
    const { characterLevel, jobTier, baseSkillLevel3rd, baseSkillLevel4th } =
      context;

    // Calculate total skill level for 3rd and 4th job
    const equipped3rdJobTotal =
      baseSkillLevel3rd +
      (equippedItem.skillLevel3rd || 0) +
      (equippedItem.skillLevelAll || 0);
    const equipped4thJobTotal =
      baseSkillLevel4th +
      (equippedItem.skillLevel4th || 0) +
      (equippedItem.skillLevelAll || 0);
    const comparison3rdJobTotal =
      baseSkillLevel3rd +
      (comparisonItem.skillLevel3rd || 0) +
      (comparisonItem.skillLevelAll || 0);
    const comparison4thJobTotal =
      baseSkillLevel4th +
      (comparisonItem.skillLevel4th || 0) +
      (comparisonItem.skillLevelAll || 0);

    // Calculate coefficient with equipped item's skill level
    const coeffWithEquipped =
      jobTier === "4th"
