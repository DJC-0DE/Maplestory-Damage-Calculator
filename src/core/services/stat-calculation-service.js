...
export class StatCalculationService {
  ...
  constructor(baseStats, weaponAttackBonus) {
    ...
    if (weaponAttackBonus !== null && weaponAttackBonus !== undefined) {
      ...
      if (typeof result !== "number" || isNaN(result) || result < 0) {
        console.error(
          "getWeaponAttackBonus returned unexpected value:",
          result,
          "- treating as 0",
        );
  ...
  addDiminishingReturnStat(statKey, value, factor) {
    const oldValue = this.stats[statKey] || 0;
    const newValue =
      (1 - (1 - oldValue / factor) * (1 - value / factor)) * factor;
    this.stats[statKey] = newValue;
    return this;
  }
...
export class CumulativeStatCalculator {
  ...
  nextStep(statKey, cumulativeIncrease, currentStep, isFlat) {
    ...
    const stepIncrease =
      cumulativeIncrease - this.seriesState.previousCumulativeIncrease;
    ...
    if (isFlat) {
      ...
      if (statKey === "statDamage") {
        ...
        currentDPS = this._stepMultiplicative(
          statKey,
          cumulativeIncrease,
          stepIncrease,
        );
        ...
        currentDPS = this._stepDiminishing(
          statKey,
          cumulativeIncrease,
          stepIncrease,
          factor,
    ...
    const marginalGain =
      ((currentDPS - this.seriesState.previousDPS) /
        this.seriesState.previousDPS) *
      100;
  ...
  _stepAttack(cumulativeIncrease, stepIncrease) {
    const effectiveIncrease =
      stepIncrease * (1 + this.seriesState.weaponAttackBonus / 100);
  ...
  _stepMultiplicative(statKey, cumulativeIncrease, stepIncrease) {
    ...
    const newValue =
      ((1 + oldValue / 100) * (1 + stepIncrease / 100) - 1) * 100;
  ...
  _stepDiminishing(statKey, cumulativeIncrease, stepIncrease, factor) {
    const oldValue = this.seriesState.cumulativeStats[statKey] || 0;
    const newValue =
      (1 - (1 - oldValue / factor) * (1 - stepIncrease / factor)) * factor;
