  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      if (applyThreshold) {
        // Apply thresholding
        const threshold = 230;
        const thresholdedData = ctx.getImageData(
          0,
          0,
          canvas.width,
          canvas.height,
        ...
        for (let i = 0; i < thresholdedPixels.length; i += 4) {
          ...
          thresholdedPixels[i] =
            thresholdedPixels[i + 1] =
            thresholdedPixels[i + 2] =
              value; // Binarize
        }
...
export async function extractText(imageURL, debug = false) {
  const preprocessedImageURL = await preprocessImage(
    imageURL,
    true,
    true,
    true,
...
export function parseBaseStatText(text) {
  ...
  const cleanData = text
    ...
    .filter((x) => x)
    .map((x) => {
      // split into [stat, value]
      if (/^\d/.test(x)) {
        // starts with digit, i.e: 1st Job Skill
  ...
  let matchedData = cleanData.flatMap((x) => {
    ...
    for (const displayNameKey of Object.keys(displayNamesToInput).sort(
